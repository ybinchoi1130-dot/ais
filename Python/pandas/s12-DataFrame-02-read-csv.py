...
#   - SSV(Space Separated Values)  구분자가 스페이스(' ')
...
#   - 옵션
#     - encoding='utf-8'
#     - index=True(기본값) or False
#     - header=True(기본값) or False
#     - sep=',' # CSV(기본값), TSV(\t), SSV(' ')
...
# 옵션: index=False, 인덱스를 제외
ndf.to_csv("학생정보-noindex.txt", index=False)

#%%

# CSV 파일 읽기
rdf = pd.read_csv("학생정보-noindex.txt")
print(rdf)
