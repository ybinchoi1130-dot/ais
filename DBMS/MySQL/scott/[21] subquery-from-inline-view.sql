...
SELECT empno, ename, deptno FROM emp WHERE deptno = 10; -- 3개 행
SELECT dname FROM dept WHERE deptno = 10; -- 1개행
...
SELECT e.empno, e.ename, e.deptno, d.dname
    FROM (SELECT empno, ename, deptno 
			FROM emp WHERE deptno = 10) e CROSS JOIN
        (SELECT dname 
...
SELECT e.empno, e.ename, e.deptno, d.deptno, d.dname
    FROM (SELECT empno, ename, deptno FROM emp WHERE deptno IN(10,30)) e,
        (SELECT deptno, dname FROM dept WHERE deptno IN(10,30)) d;
...
SELECT e.empno, e.ename, e.deptno, d.deptno, d.dname
    FROM (SELECT empno, ename, deptno FROM emp WHERE deptno IN(10,30)) e
	JOIN (SELECT deptno, dname FROM dept WHERE deptno IN(10,30)) d
    ON e.deptno = d.deptno;          
