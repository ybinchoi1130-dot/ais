...
SELECT * FROM information_schema.TABLE_CONSTRAINTS WHERE table_name = 'DEPTX';

DESC DEPTX;
-- 부모 테이블을 먼저 삭제할 수 없다.
-- 참조하고 있는 자식 테이블을 먼제 삭제한 후 부모를 삭제해야 한다.
-- MySQL 에러 예시: Error Code: 3730. Cannot drop table 'deptx' referenced by a foreign key constraint
DROP TABLE DEPTX;

-- DEPT : 부서정보
CREATE TABLE DEPTX
(	
    DEPTNO DECIMAL(2, 0),     -- 부서코드
...
CREATE TABLE EMPX
(	
    EMPNO DECIMAL(4, 0),          -- 사번
	ENAME VARCHAR(10),         -- 이름
	JOB VARCHAR(9),            -- 직무
	MGR DECIMAL(4, 0),            -- 매니저코드
	HIREDATE DATE,              -- 입사일자
	SAL DECIMAL(7, 2),            -- 급여
	COMM DECIMAL(7, 2),           -- 수당
	DEPTNO DECIMAL(2, 0),         -- 부서코드
...
INSERT INTO empx VALUES (1000, 'ABC', 'ANALYST', null, NOW(), 5000, 100, 10);
